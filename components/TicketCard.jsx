...
const TicketCard = ({ ticket }) => {
  ...
  const handleCardClick = () => {
    router.push('/detalle_ticket'); 
  };

  const handleEditClick = (e) => {
    e.stopPropagation(); 
    router.push('/actualizar_ticket'); 
  };
      ...
      {showWarning && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
            ...
            <p className="text-gray-700 mb-4">¿Estás seguro de que deseas Cerrar este Ticket?.</p>
            <input
              type="text"
              className="border border-gray-300 rounded-lg w-full py-2 px-4 mb-4"
              value={inputValue}
              onChange={handleInputChange}
              placeholder="Escribe Cerrar para confirmar"
            />
