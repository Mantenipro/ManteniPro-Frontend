...
function TicketCard({ title, description, username, date, priority, ticketId }) {
  ...
  return (
    <button
      className={`rounded-lg p-2 mb-2 shadow-md transition-all duration-200 ${isHovered ? 'bg-gradient-to-r from-[#21262D] to-[#414B66]' : 'bg-white'} 
    md:rounded-xl md:p-3 md:mb-3 md:shadow-lg`} // Para desktop
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      onClick={() => router.push(`/taskDetail/${ticketId}`)}
